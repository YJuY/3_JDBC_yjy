...
public class BoardDAO {
	...
	public int nextBoardNo(Connection conn) throws Exception{
		...
		try {
			...
			if(rs.next()) { // 조회 결과 1행 밖에 없음
				boardNo = rs.getInt(1); // 첫 번째 컬럼 값을 얻어와 boarNo에 저장
			}
		}finally {
			close(rs);
			close(pstmt);
	...
	public List<Board> searchBoard(Connection conn, int condition, String query) throws Exception{
		...
		try {
			String sql = prop.getProperty("serchBoard1")
						+ prop.getProperty("serchBoard2_" + condition)
						+ prop.getProperty("serchBoard3");
