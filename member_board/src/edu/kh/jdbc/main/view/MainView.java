...
public class MainView {
	...
	// 로그인된 회원 정보를 젖아한 객체를 참조하는 참조 변수
...
//						input = 0; //-> do-while 조건식을 false로 만듦
	...
	private void signUp() {
		...
		String memberGneder = null;

		try {
			// 아이디를 입력 받아 중복이 아닐 때 가지 반복
			while (true) {
				...
				// 중복여부를 검사하는 서비스 호추 후 결과(1/0) 반환 받기
			...
			while (true) {
				...
				memberGneder = sc.next().toUpperCase();

				if (memberGneder.equals("M") || memberGneder.equals("F")) {
					break;
				} else {
			...
			Member member = new Member(memberId, memberPw2, memberName, memberGneder);

			int result = service.sighUp(member);
