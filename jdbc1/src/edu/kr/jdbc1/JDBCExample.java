...
public class JDBCExample {
	public static void main(String[] args) {
		...
		try {
			...
			// localhost == 127.0.0.1 (loop back ip[자신아이피])
			// 115.90.212.22
			...
			// 3. SQL 작성
			// ** JAVA에서 작성되는 SQL은 마지막 ;(세미콜론)을 찍지 않아야 한다! **
			...
			while(rs.next()) {
				...
				// 현재 행의 "EMP_ID" 문자열 커럼의 값을 얻어옴
				
				String empName = rs.getString("EMP_NAME"); 
				// 현재 행의 "EMP_NAME" 문자열 커럼의 값을 얻어옴
				
				int salary = rs.getInt("SALARY");
				// 현재 행의 "SALARY" 문자열 커럼의 값을 얻어옴
