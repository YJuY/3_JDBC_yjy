...
public class LoadXMLFile {
	public static void main(String[] args) {
		...
		try {
			Properties prop = new Properties(); //#vaule가 String으로 제한된 맵 -> ``Map<String, String>
			...
			System.out.println(conn);//--> 출력되면 아 DB랑 연결은 되었구나ㅏㅏㅏ
			
			/* 왜 XML파일을 이용해서 DB 연결 저보를 일겅와야될까?
			 * 
			 * 1. 코드 중복 제거
			 * 2. 별도 관리 용도 
			 * 	 - 별도 파일을 이용해서 수정이 용이
			 * 
			 * 3. 재 컴파일을 진해아지 않기 위해서
			 * 	 - 코드가 길수록 컴파일에 소요되는 시간이 크다
			 *  	-> 코드 수정으로 인한 컴파일 소요시간을 없앰.
			 *  		(파일의 내용을 읽어오는 코드만 작성해두면
			 *  		 Java 코드 수정 없이, 파일 내용만 수정하면
			 *     		 재 컴파일은 수행되지 않는다.)
			 * 4. XML 파일에 작성된 문자열 형태를 그래로 읽어오기 때문에
			 *    SQL 작성 시 좀 더 편리해진다.
			 */
